...
int main(int argc, char* argv[]) {
    ...
    while (c != EOF) {
        ...
        while(!Accept(estado) && !Error(estado)){
            ...
            // else{
            //     if(estado != 0){
            //         buffer[index-1] = '\0'; // Terminar el string
            //         ungetc(c, archivo); // Devolver el caracter al flujo de entrada
            //     }
            // }
        }
        if(Accept(estado)){
            ...
            // printf("Estado aceptador: %d, Caracter: %c\n", estado, c);
